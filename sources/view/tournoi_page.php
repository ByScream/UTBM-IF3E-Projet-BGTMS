...
<div class="container">
    <h2>Gérer les tournois disponibles</h2>
    <?php
    session_start();
    $bdd = new PDO("mysql:host=localhost;dbname=IF3E_Projet_B;charset=utf8", "root", "");
    $req = $bdd->prepare("SELECT tournament.id, tournament.name, tournament.number_of_players, tournament.match_rules, tournament.type, users.pseudo, games.game_name FROM tournament INNER JOIN users ON users.id=tournament.owner INNER JOIN games ON games.id=tournament.game_id");
    $req->execute();
    ?>
    ...
    <form method="post" action="manage_tournament.php">
        <br>
        <label for="tournoi" class="form-label">Gérer un tournoi</label>
        ...
        <div class="col-12">
            <button class="btn btn-primary" type="submit">Gérer</button>
        </div>
    </form>
    <form method="post" action="create_tournament.php">
        <br>
        <div class="col-12">
            <button class="btn btn-primary" type="submit">Créer un tournoi</button>
        </div>
    </form>
</div>
